...
class OracleConnector:
    ...
    def upsert_records(
        ...
        Args:
            ...
            compare_cols: si se indica, solo compara estas columnas para detectar cambios.
                          Útil para evitar actualizaciones innecesarias cuando solo cambian
                          timestamps (FECHA_PRED, FECHA_ACTUALIZACION). Si es None, compara
                          todos los campos comparables (comportamiento por defecto).
        ...
        # Si compare_cols está definido, restringir la comparación a esas columnas
        if compare_cols is not None:
            compare_upper = {c.upper() for c in compare_cols}
            comparable_cols = [
                c for c in non_pk_cols
                if c in compare_upper and c not in clob_cols
            ]
        else:
