...
Flujo enriquecido (run_predictions_v2):
    ...
    2. Para cada segmento:
       ...
       f. Construye tabla PREDICCIONES_V2 (con etapa, subetapa, target_real, explicacion)
    3. UPSERT en Oracle tabla PREDICCIONES_V2 por clave (OPP_ID, ETAPA, SUBETAPA)
    4. Opcionalmente: INSERT en PREDICCIONES_V2_HIST para auditoría completa
...
Resultado en Oracle:
    ...
    PREDICCIONES_V2   — última predicción por (OPP_ID, ETAPA, SUBETAPA), con SHAP
    PREDICCIONES_V2_HIST — historial completo de PREDICCIONES_V2 (opcional)
"""
...
PREDICCIONES_V2_TABLE = "PREDICCIONES_V2"
PREDICCIONES_V2_HIST  = "PREDICCIONES_V2_HIST"
...
def construir_resultado_v2(
    ...
    """
    Construye la tabla PREDICCIONES_V2 lista para upsert en Oracle.
    ...
    Returns:
        DataFrame con columnas de PREDICCIONES_V2.
    """
...
def guardar_en_oracle_v2(df_resultado: pd.DataFrame, save_hist: bool = False) -> None:
    """
    Guarda PREDICCIONES_V2 en Oracle usando UPSERT por OPP_ID_ETAPA_COMP.

    Estrategia:
        - UPSERT: 1 fila por (OPP_ID, ETAPA, SUBETAPA); se actualiza si cambia.
        - Si save_hist=True: INSERT también en PREDICCIONES_V2_HIST (auditoría completa).

    Args:
        df_resultado: DataFrame con columnas de PREDICCIONES_V2.
        save_hist:    Si True, también inserta en PREDICCIONES_V2_HIST.
    """
    from oracle_connector import OracleConnector  # noqa: PLC0415
    ...
    logger.info(
        f"Upsert {len(records)} registros en {PREDICCIONES_V2_TABLE} "
        f"(pk=OPP_ID_ETAPA_COMP)..."
    )
    conn.upsert_records(records, PREDICCIONES_V2_TABLE, pk_col="OPP_ID_ETAPA_COMP")
    logger.info("Upsert completado.")

    if save_hist:
        logger.info(f"Insertando historial en {PREDICCIONES_V2_HIST}...")
        conn.insert_records(records, PREDICCIONES_V2_HIST)
...
def run_predictions_v2(
    ...
    Args:
        save_to_oracle: Si True, hace upsert en PREDICCIONES_V2.
        save_hist:      Si True, también inserta en PREDICCIONES_V2_HIST.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save-hist",
        action="store_true",
        help="Guarda también en PREDICCIONES_V2_HIST (solo con modo V2)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
